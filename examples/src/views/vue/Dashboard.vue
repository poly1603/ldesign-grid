...
const { containerRef } = useGrid({
  column: 12,
  cellHeight: 80,
  gap: 10,
  margin: 10,
  items: [
    { id: 'stat-1', x: 0, y: 0, w: 3, h: 1, content: '<div class="stat-card"><div class="stat-icon" style="background:#3b82f6"><svg width="20" height="20" fill="white"><rect width="20" height="20" rx="2"/></svg></div><div class="stat-info"><div class="stat-value">2,543</div><div class="stat-label">总用户</div></div></div>' },
    { id: 'stat-2', x: 3, y: 0, w: 3, h: 1, content: '<div class="stat-card"><div class="stat-icon" style="background:#10b981"><svg width="20" height="20" fill="white"><circle cx="10" cy="10" r="8"/></svg></div><div class="stat-info"><div class="stat-value">$12,345</div><div class="stat-label">总收入</div></div></div>' },
    { id: 'stat-3', x: 6, y: 0, w: 3, h: 1, content: '<div class="stat-card"><div class="stat-icon" style="background:#f59e0b"><svg width="20" height="20" fill="white"><polygon points="10,2 2,18 18,18"/></svg></div><div class="stat-info"><div class="stat-value">89%</div><div class="stat-label">活跃度</div></div></div>' },
    { id: 'stat-4', x: 9, y: 0, w: 3, h: 1, content: '<div class="stat-card"><div class="stat-icon" style="background:#ef4444"><svg width="20" height="20" fill="white"><path d="M10 2 L18 10 L10 18 L2 10 Z"/></svg></div><div class="stat-info"><div class="stat-value">156</div><div class="stat-label">新订单</div></div></div>' },
...
const { containerRef } = useGrid({
  ...
  items: [
    // 统计卡片
    { id: 'stat-1', x: 0, y: 0, w: 3, h: 1, content: statCard },
    // 图表
    { id: 'chart', x: 0, y: 1, w: 8, h: 3, content: chartWidget },
    // 活动列表
    { id: 'activity', x: 8, y: 1, w: 4, h: 3, content: activityList },
  ],
});`;
</script>
